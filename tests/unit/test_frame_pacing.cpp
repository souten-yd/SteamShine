 ...
 * @brief Tests for immediate unique output and rational duplicate pacing.
 */
#include <chrono>
#include <deque>
...
/**
 * @brief Verify an immediate unique frame replaces the independent deadline phase.
 */
TEST(FramePacingDeadline, UniqueFrameReanchorsNextDuplicate) {
  frame_pacing::deadline_pacer_t pacer {{30, 1}};
  pacer.reset(at(0ns));

  pacer.record_immediate_output(at(14ms));

  EXPECT_FALSE(pacer.poll(at(33333333ns)).due);
  EXPECT_EQ(pacer.next_deadline(), at(47333333ns));
  EXPECT_TRUE(pacer.poll(at(47333333ns)).due);
}

/**
 * @brief Verify immediate recovery output cannot be followed by a stale deadline burst.
 */
TEST(FramePacingDeadline, ForcedOutputConsumesStaleDeadline) {
  frame_pacing::deadline_pacer_t pacer {{30, 1}};
  pacer.reset(at(0ns));
  ASSERT_TRUE(pacer.poll(at(33333333ns)).due);

  pacer.record_immediate_output(at(40ms));

  EXPECT_FALSE(pacer.poll(at(66666666ns)).due);
  EXPECT_EQ(pacer.next_deadline(), at(73333333ns));
}
...
/**
 * @brief Verify callback-driven capture does not wait for an unrelated consumer phase.
 */
TEST(SyntheticPipeWireProducer, PublishesArrivalBeforeIndependentCaptureDeadline) {
  const auto producer_arrival {at(20ms)};
  frame_pacing::deadline_pacer_t obsolete_capture_clock {{60, 1}};
  obsolete_capture_clock.reset(at(7ms));

  ASSERT_GT(obsolete_capture_clock.next_deadline(), producer_arrival);
  const auto callback_driven_delivery {producer_arrival};
  EXPECT_EQ(callback_driven_delivery - producer_arrival, 0ns);
  EXPECT_EQ(obsolete_capture_clock.next_deadline() - producer_arrival, 3ms + 666666ns);
}
