...
- Latest matching validation: SteamOS Runtime Build run `30594131028`, Artifact
  `steamshine-steamos-x86_64-1801b0cdea131efeff23b8d1cb7e3dab400c10ca`
  (PR #17, merged as `95f6a2f49589c6ffff113215c143f6fb39c521ca`). Installed
  locally with a matching `BUILD_INFO.json` commit, an active service, and
  `NRestarts=0`; the Stream page and automatic client registration were accepted
  by the operator in a browser on that Artifact.
- Preceding matching validation: SteamOS Runtime Build run `30459891199`,
  Artifact `steamshine-steamos-x86_64-c0d5d61657a3895b586892d10756a7d89715af23`.
...
| Source routing | Implemented, hardware acceptance pending | Pure routing distinguishes physical Desktop, verified attached stock Gamescope, retained owned-private, new owned-private, and rejection. A verified stock source always replaces startup preflight when live KWin is absent; Desktop Big Picture uses owned fullscreen nested Gamescope when KWin is live. Re-run the complete source/backend matrix on the matching Artifact. |
| Owned private Gamescope | Implemented, hardware acceptance pending | Private runtime ownership, remote-only headless and fullscreen Wayland-nested backends, Steam idle migration, application endpoint, backend/KWin-generation retain checks, reconnect, and owned-only cleanup exist. Prior headless heads ran on hardware; matching nested and migration acceptance remains required. |
...
| Idle stock handoff | Implemented, hardware acceptance pending | Explicit `attach`/`auto_idle` policy, fail-closed game-scope assessment, owner-bound boot/PID/start-time lease, guarded target stop, owned-headless route, normal/failure restore, and stale-lease recovery exist. Automated policy, lifecycle, guard, HDR, and input regressions pass; first-connection HDR, held input, active-game attach, crash restore, cable changes, and cold boots remain hardware gates. |
...
| Per-client profiles | Implemented; store, registration and editor hardware accepted | PR F adds an owner-private, schema-versioned, atomic, 64-entry profile store keyed by paired-client ID and network class. The user explicitly activates one network class per client; IP addresses are never used to guess LAN, Wi-Fi, or overlay state. RTSP supplies a signature from current request facts, applies only safe FPS/bitrate/HDR-off bounds, records codec/HDR conflicts that yield to the client, and persists the final bounded learned start rate into the same exact active profile. PR #17 adds `ensure_registered`, which records a connecting paired client once under the neutral `default` network class with every policy automatic and no ceilings, never modifies an existing entry, and never refreshes a changed capability signature; the editor moved into a floating dialog that carries the client ID and signature from the recorded entry. Automatic first-connection registration and the editor were accepted by the operator in a browser against Artifact `steamshine-steamos-x86_64-1801b0cdea131efeff23b8d1cb7e3dab400c10ca`. Application of geometry/quality/orientation/safe-area selections to the pipeline is still not implemented and therefore not covered by this acceptance. |
| Negotiation Web UI | Implemented and hardware accepted | PR #17 replaces the raw four-stage dump with a banner, bitrate/FPS/latency/loss tiles, a delivered-settings section, a connection-quality section, and fallback reasons shown only on divergence, keeping two-second bounded polling, media-path non-ownership, and the direct Sunshine fallback route. Accepted by the operator in a browser against Artifact `steamshine-steamos-x86_64-1801b0cdea131efeff23b8d1cb7e3dab400c10ca` (run `30594131028`), installed locally with matching `BUILD_INFO.json` commit, active service, and `NRestarts=0`. The advanced-settings page was removed and its navigation slot now reaches the virtual-display policy. |
