...
namespace platf::virtualhid {
  ...
  namespace {
    ...
    /**
     * @brief Preserve the target seat in Linux virtual-device names.
     * @param profile Device profile whose friendly name is scoped.
     */
    void scope_device_to_seat(lvh::DeviceProfile &profile) {
#if defined(__linux__) || defined(__FreeBSD__)
      const auto seat = input_seat::get_target_seat();
      if (!seat.empty() && seat != "seat0") {
        profile.name += " (" + seat + ")";
      }
#endif
    }
...
#if defined(__linux__) || defined(__FreeBSD__)
    desktop_mouse_initialized = true;
#endif
    mouse.reset();
...
#if defined(__linux__) || defined(__FreeBSD__)
    const std::array<std::uint32_t, 5> buttons {BTN_LEFT, BTN_MIDDLE, BTN_RIGHT, BTN_SIDE, BTN_EXTRA};
