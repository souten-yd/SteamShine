...
namespace config {
  ...
  struct steamos_virtual_display_t {
    bool enabled {false};  ///< Enable custom startup probing; client app launches retain the owned-session invariant.
    steamos_virtual_session::virtual_display_mode_e mode {steamos_virtual_session::virtual_display_mode_e::auto_detect};  ///< Startup encoder-probe display policy parsed during configuration loading.
    steamos_virtual_session::session_source_policy_e session_source {steamos_virtual_session::session_source_policy_e::auto_select};  ///< Startup encoder-probe Gamescope source policy.
    ...
    steamos_virtual_session::stock_handoff_policy_e stock_session_handoff {steamos_virtual_session::stock_handoff_policy_e::attach};  ///< Legacy parsed value retained for configuration compatibility.
    ...
    int pipewire_node_timeout_milliseconds {10000};  ///< Maximum wait for a Gamescope PipeWire capture-output node.
