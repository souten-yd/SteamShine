...
No monitor. No dummy HDMI plug. Core streaming installs without `sudo`. Close the lid on your Deck, pick it up in Moonlight from
...
The Display page controls startup encoder probing. A real Moonlight application launch is stricter:
SteamShine stops stock Game Mode when it is running and starts (or reuses) its owned Gamescope at the
client-requested geometry, regardless of monitor connection. A missing stock session is a successful
no-op. The startup choices are:

| `steamos_virtual_display_mode` | What happens |
| --- | --- |
| `off` | Stock Sunshine behaviour. Captures a real display. |
| **`auto`** *(default)* | During startup probing, use a verified capturable source; client launches still use owned Gamescope. |
| `force` | Always private, even with a monitor plugged in. Never falls back. |

| `steamos_session_source` | What happens |
| --- | --- |
| **`auto`** *(default)* | During startup probing, attach to verified Game Mode when available; client launches still use owned Gamescope. |
...
- **OS integrations are deliberately narrow.** Decky lifecycle control, automatic Decky activation,
  and `/dev/uinput`/`/dev/uhid` rules require one interactive `./steamshine.sh repair` so the installer
  can place a root-owned helper and an exact-argument sudoers policy. The Web service never receives
  general sudo authority. Core streaming remains a user installation when these integrations are skipped.
