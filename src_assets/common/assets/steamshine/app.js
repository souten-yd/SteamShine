...
function storageRecoveryCard(storage) {
  const rows = (storage.volumes || []).map((volume) => `<div class="section"><h4>${escapeHtml(volume.label || volume.uuid)} · ${formatBytes(volume.size_bytes || 0)}</h4><div class="rows">
