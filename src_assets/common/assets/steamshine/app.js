...
async function renderAuthenticated(session) {
  ...
  const steamMigration = status.app_launch_rejected_reason
    ? `<p class="notice"><strong>Application launch rejected (${escapeHtml(status.app_launch_rejected_reason)}).</strong> ${escapeHtml(status.app_launch_rejected_message || 'See the service log for details.')}${status.migration_required ? ' Explicit Desktop Steam migration is required; SteamShine will never stop it without confirmation.' : ''}</p>`
    : '';
  shell(`<section><h2>Welcome, ${escapeHtml(session.username)}</h2>${steamMigration}<div class="grid"><div class="card"><div>Service launch</div><div class="metric">${escapeHtml(status.service_launch_mode || 'unknown')}</div></div><div class="card"><div>Binary commit</div><div class="metric">${escapeHtml(status.service_binary_commit || 'unknown')}</div></div><div class="card"><div>Config path</div><div class="metric">${escapeHtml(status.service_config_path || 'unknown')}</div></div><div class="card"><div>Active streams</div><div class="metric">${escapeHtml(status.active_streams)}</div></div><div class="card"><div>Application</div><div class="metric">${status.application_running ? 'Running' : 'Idle'}</div></div><div class="card"><div>Gamescope</div><div class="metric">${status.gamescope_active ? 'Active' : 'Idle'}</div></div><div class="card"><div>Input target</div><div class="metric">${escapeHtml(status.input_route_target || 'unknown')}</div></div><div class="card"><div>Input route error</div><div class="metric">${escapeHtml(status.input_route_error || 'none')}</div></div><div class="card"><div>Input queue current / max</div><div class="metric">${escapeHtml(`${status.input_queue_current || 0} / ${status.input_queue_max || 0}`)}</div></div><div class="card"><div>Motion coalesced / dropped</div><div class="metric">${escapeHtml(`${status.input_motion_coalesced || 0} / ${status.input_motion_dropped || 0}`)}</div></div><div class="card"><div>Virtual display mode</div><div class="metric">${status.virtual_display_enabled ? escapeHtml(status.virtual_display_mode) : 'Disabled'}</div></div><div class="card"><div>Capture selection</div><div class="metric">${escapeHtml(status.capture_selection_reason || 'unknown')}</div></div><div class="card"><div>Session origin</div><div class="metric">${escapeHtml(status.virtual_display_origin || 'none')}</div></div><div class="card"><div>Steam location</div><div class="metric">${escapeHtml(status.steam_location || 'unknown')}</div></div><div class="card"><div>Virtual display state</div><div class="metric">${escapeHtml(status.virtual_display_state || 'Disabled')}</div></div><div class="card"><div>Private socket</div><div class="metric">${escapeHtml(status.virtual_display_socket || 'Unavailable')}</div></div><div class="card"><div>Gamescope PID</div><div class="metric">${escapeHtml(status.gamescope_pid > 0 ? status.gamescope_pid : 'Unavailable')}</div></div><div class="card"><div>Render node</div><div class="metric">${escapeHtml(status.render_node || status.game_gpu || 'Auto')}</div></div><div class="card"><div>Capture frames</div><div class="metric">${escapeHtml(status.captured_frames || 0)}</div></div><div class="card"><div>Encoded packets</div><div class="metric">${escapeHtml(status.encoded_packets || 0)}</div></div><div class="card"><div>Encoder</div><div class="metric">${escapeHtml(status.encoder || 'Auto')}</div></div></div></section>`, true);
}
